...
// src/controllers/authController.js
// ⚠️  EMAIL VERIFICATION BYPASSED — auto-verifies all users on register
// When email is configured, restore isVerified: false and re-enable OTP flow

const bcrypt  = require('bcryptjs');
const jwt     = require('jsonwebtoken');
const prisma  = require('../config/prisma');
const {
  generateAccountNumber, success, error,
} = require('../utils/helpers');
...
const register = async (req, res, next) => {
  try {
    ...
    // Create user (auto-verified) + wallet + bank account
    const user = await prisma.$transaction(async (tx) => {
      const u = await tx.user.create({
        data: {
          fullname,
          ...
          password: hashed,
          bvn: bvn || null,
          isVerified: true, // bypass — remove when email is configured
        },
      });
      await tx.wallet.create({ data: { userId: u.id } });
      await tx.bankAccount.create({
    ...
    return success(res, {
      token,
      user: { id: user.id, fullname, email, phone, isVerified: true, role: user.role },
      account: { accountNumber, bankName: process.env.BANK_NAME || 'Fintech Bank' },
...
// ── POST /api/auth/verify-email ───────────────────────────────
// Stub — always returns success since verification is bypassed
const verifyEmail = async (req, res) => {
  return success(res, {}, 'Email verification is currently disabled.');
};

// ── POST /api/auth/resend-otp ─────────────────────────────────
// Stub — no-op while email is disabled
const resendOTP = async (req, res) => {
  return success(res, {}, 'Email service is currently disabled.');
};
...
const forgotPassword = async (req, res, next) => {
  try {
    ...
    // OTP flow disabled — uncomment when email is configured
    return success(res, {}, 'If that email exists, a reset link has been sent.');
  } catch (err) {
    next(err);
  }
...
// ── POST /api/auth/reset-password ────────────────────────────
// Stub — requires email OTP which is disabled
const resetPassword = async (req, res) => {
  return error(res, 'Password reset via email is currently disabled. Contact support.', 503);
};
